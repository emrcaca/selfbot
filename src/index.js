...
function spawnSelfbotProcess(token, index) {
    ...
    childProcess.stderr.on('data', (data) => {
        Loggers.Main.error(`Selfbot (User ID: ${userId || 'Unknown'} - Index: ${index}) stderr: ${data.toString()}`);
    });
    ...
    childProcess.on('exit', (code, signal) => {
        if (userId) {
            ...
            Loggers.Main.info(`Selfbot process exited (User ID: ${userId}, Code: ${code}, Signal: ${signal})`);
        }
    });
...
function handleNotifierMessage(message) {
    const forwardableTypes = ['komut_kullanildi', 'setch_command'];
