...
function handleCommand(message) {
    try {
        ...
        if (process.send) {
            ...
            // Send farm status update based on result message
            let isChannelFarming = false;
            let isPermanentFarming = false;

            if (resultMessage.includes('Farm started in this channel')) {
                isChannelFarming = true;
            } else if (resultMessage.includes('Farm stopped')) {
                isChannelFarming = false;
            } else if (resultMessage.includes('Farming enabled for permanent channels')) {
                isPermanentFarming = true;
            } else if (resultMessage.includes('Farming disabled for permanent channels')) {
...
function handleFarmCommand(message) {
    ...
    switch (farmType) {
        ...
        case 'permanent_channels':
            return handlePermanentFarm(targetUserId);
 ...
 * @returns {string} Result message
 */
function handlePermanentFarm(userId) {
