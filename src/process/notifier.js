...
process.on('message', (message) => {
  ...
  if (message.type === 'komut_sonucu' && message.interactionId) {
    ...
    userFarmStates.set(message.userId, {
      ...currentState,
      isChannelFarming: message.isChannelFarming,
      isPermanentFarming: message.isPermanentFarming
    });
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'selfbot') {
        ...
        if (commandType === 'farm') {
          ...
          const userState = userFarmStates.get(interaction.user.id) || { isChannelFarming: false, isPermanentFarming: false };

          const farmComponents = generateFarmControlComponents(userState.isChannelFarming, userState.isPermanentFarming, true);
      ...
      if (interaction.customId === 'dismiss_alert') {
        ...
        // Kullanıcının farm durumunu güncelle
        ...
        userFarmStates.set(interaction.user.id, {
          isChannelFarming: farmType === 'this_channel' ? isChannelFarming : currentUserState.isChannelFarming,
          isPermanentFarming: farmType === 'permanent_channels' ? isPermanentFarming : currentUserState.isPermanentFarming
        });
